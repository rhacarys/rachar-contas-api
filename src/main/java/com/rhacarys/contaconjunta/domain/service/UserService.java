...
public class UserService {
    ...
    public UserResponse updateMe(UserUpdateRequest request, User loggedUser) {
        log.debug("Starting user profile update for userId: {}", loggedUser.getId());
        ...
        log.info("User profile updated successfully - userId: {}, newName: {}",
     ...
     * Account is anonymized rather than fully deleted to preserve financial
     * history.
     */
    @Transactional
    public void deleteMe(User loggedUser) {
        log.debug("Starting account deletion for userId: {}", loggedUser.getId());
        ...
        log.info("User account deleted (anonymized) - userId: {}, login: {}",
    ...
    private void updatePasswordIfNeeded(User user, UserUpdateRequest request) {
        if (request.newPassword() == null || request.newPassword().isBlank()) {
            log.debug("No password update requested for userId: {}", user.getId());
            return;
        }

        if (request.currentPassword() == null || request.currentPassword().isBlank()) {
            log.warn("Password update attempted without current password for userId: {}", user.getId());
            throw new BusinessException("Current password is required to set a new one", HttpStatus.BAD_REQUEST);
        }

        if (!passwordEncoder.matches(request.currentPassword(), user.getPassword())) {
            log.warn("Password update failed - current password mismatch for userId: {}", user.getId());
            throw new BusinessException("Current password does not match", HttpStatus.BAD_REQUEST);
        }

        user.setPassword(passwordEncoder.encode(request.newPassword()));
        log.info("User password updated successfully - userId: {}", user.getId());
    }

    /**
     * Validates that user has zero balance in all parties before allowing account
     * deletion.
    ...
    private void validateNoPendingBalances(User user, List<Membership> memberships) {
        log.debug("Validating zero balance across {} memberships for userId: {}",
        ...
        for (Membership membership : memberships) {
            ...
            if (hasPendingBalance) {
                log.warn("Account deletion blocked - pending balance in party: {}, userId: {}",
    ...
    private void anonymizeAccount(User user, List<Membership> memberships) {
        ...
        log.debug("Account anonymized - userId: {}", user.getId());
    }
}